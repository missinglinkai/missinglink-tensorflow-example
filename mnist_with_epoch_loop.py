...
from missinglink_kernel.callback.tensorflow_project import TensorFlowProject
...
def run_training():
    ...
    with tf.Graph().as_default():
        ...
        with project.create_experiment(
            ...
            # Use `experiment.epoch_loop` and `experiment.batch_loop` to manage the training
            # loops with epochs and batches.
            # - The indexes `epoch` and `batch` produced by these generator are 0-based.
            for epoch in experiment.epoch_loop(EPOCHS):
                for batch in experiment.batch_loop(total_batches):
                    ...
                    with experiment.train(custom_metrics={'batch': lambda: batch}):
                        # Notice that you only need to provide the optimizer op. The SDK will automatically run
